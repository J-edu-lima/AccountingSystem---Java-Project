...
public class TransactionMapper {
	...
	public static TransactionRequestDTO toDto(Transaction transaction, Company company) {
		return new TransactionRequestDTO(transaction.getValue(), transaction.getType(), transaction.getGenerationDate(), company);
	}

	public static Transaction toEntity(TransactionRequestDTO dto, Company company) {
		return new Transaction(dto.entryValue(), dto.type(), dto.generationDate(), company);
	}

	public static TransactionResponseDTO toDtoResponse(Transaction transaction, Long companyId) {
		return new TransactionResponseDTO(transaction.getValue(), transaction.getType(), transaction.getGenerationDate(), companyId);
	}
